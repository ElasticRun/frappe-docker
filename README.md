...
2. `k8s` - Directory that holds the configurations for Kubernetes resources required for frappe application. This creates storage classes, persistent volumes, persistent volume claims and deployments required.
    ...
    * `01-er-frappe-mariadb-endpoint.yaml` - Defines the endpoint to which the mariadb service connects to. This is the physical address of the destination mariadb instance (IP:port).
        ...
        ***Note for local deployments** - The Minikube installation uses a VM running on the developer machine. If the mariadb instance is running on the host machine, it will not be availalbe on the VM that runs the kubernetes cluster. To make this service available, SSH tunnel can be setup between host and VM using following command. Multiple -R flags can be provided to create multiple tunnels.*
...
### Step 3 - Deployment
Once the configuration files are updated for your own application's deployment, `setup.sh` script can be executed. Please do not chnage the numerical values included in the file names, as these decide the order in which the files are executed. The setup script will deploy all the required resources in pre-defined sequence and will make the application available as a service exposed on the defined port.
